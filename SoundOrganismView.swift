...
struct SoundOrganismView: View {
        ...
        TimelineView(.periodic(from: Date(), by: 0.016)) { timeline in
            Canvas { context, size in
                ...
                if smoothedAmplitude < 0.01 {
                    ...
                    drawPetalShape(
                        context: context, center: center, size: size, rotationAngle: rotationAngle)
                }
            }
        }
    ...
    private func drawPetalShape(
        context: GraphicsContext, center: CGPoint, size: CGSize, rotationAngle: Double
    ) {
